...
def run(command: str):
    ...
    proc = subprocess.run(["python3", str(SCRIPT)], input=event, text=True, capture_output=True, check=True)
...
class GuardBypassTests(unittest.TestCase):
    def test_blocks_chained_and_git_option_remote_writes(self):
        for command in ("echo ok && git push origin main", "git -C . push origin main", "printf ok; gh pr create --title x"):
            out = run(command)
            self.assertEqual(out["hookSpecificOutput"]["permissionDecision"], "deny")

    def test_blocks_shell_and_runtime_secret_reads(self):
        for command in ("source .env", "python3 -c \"print(open('.env').read())\"", "node -e \"require('fs').readFileSync('.npmrc')\""):
            out = run(command)
            self.assertEqual(out["hookSpecificOutput"]["permissionDecision"], "deny")
