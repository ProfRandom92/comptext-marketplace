...
if (!sourcePath || !contractPath) {
  console.error('usage: node build-context-ir.mjs <source-summary.json> <contract.json>');
  process.exit(2);
}
...
if (!Array.isArray(contract.required_field_paths) || contract.required_field_paths.length === 0) {
  throw new Error('required_field_paths must be non-empty');
}

const uniqSorted = values => [...new Set(values)].sort();
const orderedUnique = values => [...new Set(values)];
const sortEdges = values => [...values].sort((a,b) =>
...
const preimage = {
  schema: 'comptext-context/v1',
  source_digest: source.source_digest,
  strategy: contract.strategy,
  required_field_paths: required,
  satisfied_field_paths: satisfied,
  missing_field_paths: missing,
  constraints: uniqSorted(contract.constraints ?? []),
  required_order: orderedUnique(contract.required_order ?? []),
  dependency_edges: sortEdges(contract.dependency_edges ?? []),
  blockers: sortEdges(contract.blockers ?? []),
  recovery_paths: sortEdges(contract.recovery_paths ?? []),
  validation: {
    valid: missing.length === 0,
    failure_labels: missing.length ? ['MISSING_REQUIRED_FIELD'] : []
  },
...
const context = {
  ...preimage,
  context_id: sha256Canonical(preimage)
};
