...
const pick = (obj, keys) => Object.fromEntries(keys.filter(k => k in obj).map(k => [k, obj[k]]));const proof = {
  ...
  arms: (result.arm_results ?? []).map(a => ({
    id: a.arm_id,
    status: a.status,
    evidence: (a.evidence ?? []).map(v => v.ref).filter(Boolean)
  })),
  quality: { raw: pick(b,q), comptext: pick(x,q), delta: pick(d,q) },
  efficiency: {
    raw: pick(b,e), comptext: pick(x,e), delta: pick(d,e),
    token_reduction_ratio: c.efficiency?.token_reduction_ratio ?? null
  },
...
if (output) {
  ...
  fs.writeFileSync(output, text, { flag: 'w' });
}
